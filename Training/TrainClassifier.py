...
# This function reads the audio files from the specified folders, extracts the class labels from the folder names, and returns two lists: one containing the file paths of the audio samples and another containing the corresponding class labels as integers.
...
# This function generates a formatted string that displays the accuracy for each class in a tabular format, which can be printed to the console using tqdm.write for better visualization during training and validation.
...
# This function runs a full validation pass over the validation dataset, calculating the average loss and counting the number of correct predictions for each class to compute per-class accuracy. 
# It returns the average validation loss, an array of correct predictions per class, and an array of total samples per class.
def _run_validation(model, loader, loss_fn, n_classes):
    ...
    with torch.no_grad():
        for X_batch, y_batch in loader:
            ...
            # Accumulate total loss for averaging
